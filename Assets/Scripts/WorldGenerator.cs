...
public class WorldGenerator : MonoBehaviour
{
    ...
    private IEnumerator SpawnRegionTotemRoutine()
    {
        ...
        // Two-pass search: first restrict to "summer" (grass) cells only —
        // no beach sand, no snow peaks, no rock slopes. If the region really
        // has no grass tile at all (e.g. all-desert biome + all-mountains),
        // fall back to any flat above-water spot on the second pass.
    ...
    // Mirrors the biome classification used by PaintTerrainRoutine so the
    // region location only lands on cells that get painted as grass ("summer"
    // green). Uses normalized terrain-space coords (0-1) and the sampled world
    // Y so we can reject snow peaks, sand beaches, and rock slopes.
