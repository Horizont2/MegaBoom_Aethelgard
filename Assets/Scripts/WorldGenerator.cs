...
public class WorldGenerator : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (isRegionMission && curRegion != null)
        {
            ...
            {
                // «¿’ŒœÀ≈Õ»…: ¡ÂÂÏÓ Á·ÂÂÊÂÌËÈ seed, ˘Ó· Ï‡Ô‡ ·ÛÎ‡ ≤ƒ≈Õ“»◊ÕŒﬁ
                mapSeed = PlayerPrefs.GetInt("RegionSeed_" + curRegion.regionID, UnityEngine.Random.Range(0, 999999));
            }
            else
            {
                // Õ≈ «¿’ŒœÀ≈Õ»…: √ÂÌÂÛ∫ÏÓ ÌÓ‚ËÈ seed. ﬂÍ˘Ó Á‡ıÓÔËÏÓ - ‚≥Ì Ì‡Á‡‚Ê‰Ë ÎË¯ËÚ¸Òˇ ‚ Ô‡Ï'ˇÚ≥
    ...
    private IEnumerator PopulateBiomesRoutine()
    {
        ...
        for (int i = 0; i < spawnAttempts; i++)
        {
            ...
            catch (System.Exception e) { Debug.LogError($"[œÓÏËÎÍ‡ „ÂÌÂ‡ˆ≥ø ÔÂÙ‡·Û]: {e.Message}"); }
        }
    }
